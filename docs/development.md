...
- 预发行后缀：
  ...
  - 示例：`0.0.3a1`、`0.0.3b2`、`0.0.3rc1`
- 稳定版：去掉预发行后缀，如 `0.0.3`
- 比较顺序（从低到高）：`0.0.3a1 < 0.0.3a2 < 0.0.3b2 < 0.0.3rc1 < 0.0.3`

### 🚧 预发行策略（默认不自动发布到 TestPyPI）
- 通过 Git 标签发布预发行：`vX.Y.Z[a|b|rc]N`，例如 `v0.0.3b2`
...
### 📝 操作清单（速查）
- 预发行：
  1. 在 `src/claude_notifier/__version__.py` 设定预发行号（如 `0.0.3b2`）
  2. 打 `v0.0.3b2` 标签并创建 Release
